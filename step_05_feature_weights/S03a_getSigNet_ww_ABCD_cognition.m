%% ABCD cognition: W-W network-level permutation test 
clear;
...
for i_perm = 1:nPerm
    ...
    for i_fold = 1:length(fold_files)
        ...
        if ~isfield(tmp, 'w_Brain_Haufe')
            error('The variable w_Brain_Haufe is not present in the %s file.', fold_files{i_fold});
        end
        this_w = tmp.w_Brain_Haufe(:)';
        if i_fold == 1
            if numel(this_w) ~= nEdge
                error('Time %d, fold %d: inconsistent number of edges (%d vs %d)', ...
