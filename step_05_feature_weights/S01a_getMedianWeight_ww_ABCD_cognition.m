%%
clear;
proj_dir =  '/ibmgpfs/cuizaixu_lab/congjing/WM_prediction';
addpath('/ibmgpfs/cuizaixu_lab/congjing/toolbox/PANDA_1.3.0_64');
addpath([proj_dir '/ABCD/code/6th_post_prediction']);

current_set = 'ABCDcog';
cog_str = 'nihtbx_totalcomp';
data_dir = [proj_dir '/ABCD/code/5th_prediction/PLS_prediction/networkFC/cognition_nosmooth_motion2runFD/' cog_str];

out_dir = [proj_dir '/ABCD/results/figure/weight'];
save_name = [out_dir '/' current_set '_Haufe_FCmatrix_ww_Schaefer100'];

%% get median feature weight for each FC
Results_Cell = g_ls([data_dir '/RegressCovariates_RandomCV/Time_*/WWFC/Fold_*_Score.mat']);
for i_cv = 1:length(Results_Cell)
  tmp = load(Results_Cell{i_cv});
  w_Brain_Overall_101CV(i_cv, :) = tmp.w_Brain_Haufe;
end

%% get positive and negative feature weight proportion for each FC, set threshold at 80%

w_Brain_median_101CV = median(w_Brain_Overall_101CV,1);

%% save results
VisualizeFolder = [data_dir '/WeightVisualize_Overall_RandomCV'];
mkdir(VisualizeFolder);
save([VisualizeFolder '/w_Brain_median_101CV_WW_Haufe.mat'], 'w_Brain_median_101CV');

%% convert from FC vector to FC matrix 
lower_tri_vec = w_Brain_median_101CV;   
m = 68; n = 68;
if numel(lower_tri_vec) ~= m*(m-1)/2
    error('length mismatch: expected %d, got %d', m*(m-1)/2, numel(lower_tri_vec));
end

FCmatrix = zeros(m, n);
[ri, ci] = find(tril(true(m, n), -1));      
[~, ord] = sortrows([ri, ci], [1 2]);        
lin_idx = sub2ind([m n], ri(ord), ci(ord));  
FCmatrix(lin_idx) = lower_tri_vec(:);
FCmatrix_full = FCmatrix + FCmatrix';

save([save_name '.mat'], 'FCmatrix', 'FCmatrix_full');

%% sorted FCmatrix by networks and imagesec 
load cmap0.mat
load JHU68_info_rename.mat;
FCmatrix_rearranged = FCmatrix_full(regionID_sortedByTracts, regionID_sortedByTracts);

figure('Position', [100, 100, 550, 500]);
imagesc(FCmatrix_rearranged); 
cb = colorbar;
cb.FontSize = 14;
cb.FontName = 'Arial';
colormap(cmap);
caxis([-max(FCmatrix_rearranged(:)), max(FCmatrix_rearranged(:))]);

%% add network lines
hold on;
% Define the positions (between rows/columns) where separator lines are needed
split_positions = [10, 15, 34, 50]; 
% Draw horizontal lines between specified rows
for i = 1:length(split_positions)
    y = split_positions(i) + 0.5;
    line([0.5, 68.5], [y, y], 'Color', 'k', 'LineWidth', 0.5);
end
% Draw vertical lines between specified columns
for i = 1:length(split_positions)
    x = split_positions(i) + 0.5;
    line([x, x], [0.5, 68.5], 'Color', 'k', 'LineWidth', 0.5);
end
hold off;

%% add network legends
xtick_positions =   [5, 13, 25, 43, 59]; 
xticks(xtick_positions);
xtick_labels = tractsName;
xticklabels(xtick_labels);
ytick_positions =  [5, 13, 25, 43, 59]; 
yticks(ytick_positions);
ytick_labels = tractsName;
yticklabels(ytick_labels);
ax = gca;
ax.XTickLabelRotation = 90;
title(['W-W weight matrix of ' current_set]);
ax.XAxis.FontSize = 14; ax.YAxis.FontSize = 14; 

% save figure
saveas(gcf, [save_name '.fig']); 
saveas(gcf, [save_name '.jpg']); 
print(gcf, [save_name '.pdf'], '-dpdf', '-bestfit');
