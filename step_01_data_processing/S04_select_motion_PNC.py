...
SUBLIST = "/ibmgpfs/cuizaixu_lab/congjing/WM_prediction/PNC/results/data_selection/rest_subid_singleband.txt"

# fMRIPrep dir
BASES = [
    "/ibmgpfs/cuizaixu_lab/xuhaoshu/WM_prediction/datasets/PNC/fmriprep",
    "/ibmgpfs/cuizaixu_lab/congjing/WM_prediction/PNC/results/fmriprep",
]

TASK = "rest"
MAX_RUNS = 1
SESSION = "ses-PNC1"
OUTPUT = "/ibmgpfs/cuizaixu_lab/congjing/WM_prediction/PNC/results/head_motion/fd_summary.csv"
PASSED_TXT_OUT = "/ibmgpfs/cuizaixu_lab/congjing/WM_prediction/PNC/results/subid_final.txt"

TASK_PATTERN_FMT = "task-{}"
RUN_PATTERN = re.compile(r"run-(\d{1,2})")

# remain singleband
SINGLEBAND_KEYWORD = "singleband"
    ...
    for base in bases:
        for subj in subj_variants:
            ...
            for path in search_root.rglob("*desc-confounds_timeseries.tsv"):
                ...
                # Must be in the func directory
                if "func" not in [p.name for p in path.parents]:
                    continue

                # The filename must contain rest
                ...
                # The path must contain singleband
                full_lower = str(path).lower()
                if SINGLEBAND_KEYWORD not in full_lower:
            
                    continue  

                # Further confirmation that subid is in the filename
...
def summarize_subject(
    ...
    kept = int(sum(keeps))
    result["check_has1run"] = 1 if kept >= 1 else 0
    result["check_has2runs"] = 1 if kept >= 2 else 0
    return result
...
def main():
    ...
    cols += ["check_has1run", "check_has2runs"]
    df = df[cols]
    ...
    # Selected subjects
    if MAX_RUNS == 1:
        mask = (pd.to_numeric(df.get("rest1_keep", 0), errors="coerce").fillna(0) == 1)
        criterion_used = "rest1_keep==1 (single-run)"
    else:
        has_r1r2 = ("rest1_keep" in df.columns) and ("rest2_keep" in df.columns)
        if has_r1r2:
            mask = (pd.to_numeric(df["rest1_keep"], errors="coerce").fillna(0) == 1) & \
                   (pd.to_numeric(df["rest2_keep"], errors="coerce").fillna(0) == 1)
    ...
    print(f"[INFO] Only using files with TASK='{TASK}' and containing '{SINGLEBAND_KEYWORD}' in path")  # NEW: log
