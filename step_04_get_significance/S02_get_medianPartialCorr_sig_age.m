...
resultsFolder= [ProjectFolder '/post_prediction/get_significance/Age']; 
load([resultsFolder '/perm_partial_results_total_5fold_sortedByCVtimes.mat']);
load([ProjectFolder '/post_prediction/age_boxplot/age_dataCell_totalSets_forBoxplot.mat']);

%% get perm results for GG,GW,WW 
for i_str = 1:length(targetStr_total)
    thisTarget = targetStr_total{i_str};
   %% GG
    MedianCorr_GG = median(dataCell_totalSets{i_str+1, 2});       %obs:The median of Mean_Corr in 101 actual prediction
   ...
   %% GW
    MedianCorr_GW = median(dataCell_totalSets{i_str+1, 3});
   ...
   %% WW
    MedianCorr_WW = median(dataCell_totalSets{i_str+1, 5}); 
...
for i_str = 1:length(targetStr_total)
    thisTarget = targetStr_total{i_str};
    %% partial GW | GG
    MedianPartialCorr_GW = median(dataCell_totalSets{i_str+1, 4}, 'omitnan');
    ...
    elseif MedianPartialCorr_GW<0
        ...
        error('MedianPartialCorr_GW == 0, Target=%s（不应出现）', thisTarget);
    end
    perm_partialP_GW_totalStr(i_str,1) = perm_partialP_GW;

   %% partial WW | GG
    MedianPartialCorr_WW = median(dataCell_totalSets{i_str+1, 6}, 'omitnan');
