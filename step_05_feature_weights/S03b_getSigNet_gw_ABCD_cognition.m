%% ABCD cognition: G-W network-level permutation test 
clear;
...
if numel(w_obs_vec) ~= nEdge
    error('GW edge error：nEdge = %d, but 100×68 = %d', numel(w_obs_vec), nEdge);
end
...
for i_perm = 1:nPerm
    ...
    if isempty(fold_files)
        error('Time %d (%s) not have GWFC/Fold_*_Score.mat', ...
    ...
    % vector -> 100x68（Python row-major）
