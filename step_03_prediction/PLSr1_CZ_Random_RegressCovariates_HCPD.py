...
def PLSr1_KFold_RandomCV(Subjects_Data_List, Subjects_Score, Covariates, Fold_Quantity, ComponentNumber_Range, CVRepeatTimes_ForInner, ResultantFolder, Parallel_Quantity, Permutation_Flag, RandIndex_File=''):
    ...
    if len(RandIndex_File) == 0:
        # MARK: Stratified sampling: to make the y_label distributions of different datasets as similar as possible.
...
# ========= Regression covariates:
            ...
            # First, construct training and testing DataFrames, using the same column names: Covariate_0/1/2...
            df = pd.DataFrame({
            ...
            # Covariates = [sex, motion, site],
            # Therefore, columns 0 and 2 are categorical variables (sex, site), and column 1 is a continuous variable (motion).
            categorical_idx = [0, 2]

            terms = []
            for k in range(Covariates_Quantity):
                if k in categorical_idx:
                    # sex and site are used as categorical variables using C().
                    terms.append(f'C(Covariate_{k})')
                else:
                    # Other covariates as continuous variables
        ...
        Fold_J_Corr_List = []
        Fold_J_MAE_List = []
        # Cross-validation is performed based on the hyperparameters of the inner CV.
        for conn_index in np.arange(len(Subjects_Data_List)):
